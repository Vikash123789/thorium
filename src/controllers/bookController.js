...
const createBooks = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length == 0 || data == null) {
      return res.status(400).send({ status: false, msg: "No input provided by user", });
    }
    ...
    if (!validation.valid(excerpt)) {
      return res.status(400).send({ status: false, msg: "No excerpt provided by user", })
    }


    if (!validation.valid(userId)) {
      return res.status(400).send({ status: false, msg: "No userId provided by user", })
    }

    if (!validation.isValidObjectId(userId)) {
      return res.status(400).send({ status: false, msg: "Not  Valid Object Id", });
    }
    ...
    if (!validation.isValidIsbn(ISBN)) {
      return res.status(400).send({ status: false, message: "Enter  valid 12 digit ISBN number " });
    }
    ...
    if (!validation.valid(category)) {
      return res.status(400).send({ status: false, msg: "No category provided by user", })
    }

    if (!validation.valid(subcategory)) {
      return res.status(400).send({ status: false, msg: "No subcategory provided by user", })
    }

    if (!validation.valid(releasedAt)) {
      return res.status(400).send({ Status: false, msg: "Please provide the realesdAt field and enter a date" })
    }

    if (!validation.isValidDateFormat(releasedAt)) {
      return res.status(400).send({ Status: false, msg: "Please enter the date in correct YYYY-MM-DD format" })
    }
    ...
    } else { return res.status(403).send({ Status: false, msg: "User is not authorized" }) }
...
const getBooks = async (req, res) => {
  try {
    ...
    if (validation.valid(userId)) {
      if (!validation.isValidObjectId(userId)) { return res.status(400).send({ Status: false, msg: "Please enter valid user id" }) }
    ...
    if (findData.length == 0) {
      return res.status(404).send({ Status: false, msg: "No books with the respected query were found" })
    }
...
const getById = async (req, res) => {
  try {
    ...
    if (!findId) {
      return res.status(404).send({ status: false, msg: "No book found with this Book id" })
    }

    if (findId.isDeleted == true) {
      return res.status(400).send({ status: false, msg: "The book has been  deleted" })
    }
...
const updateBooks = async (req, res) => {
  try {
    ...
    if (!findId) {
      return res.status(404).send({ status: false, msg: "No book with the given id exists", })
    }
    if (findId.isDeleted == true) {
    ...
    if (findId.userId == req.decodedToken.userId) {
      ...
      if (validation.valid(title)) {
        ...
        if (duplliTitle) { return res.status(400).send({ Status: false, msg: "This title already exists. Please enter a different title" }) }
        filter["title"] = title
      }

      if (validation.valid(ISBN)) {
        if (!validation.isValidIsbn(ISBN)) { return res.status(400).send({ Status: false, msg: "Please entr a valid ISBN" }) }
        let dupliIsbn = await bookModel.findOne({ ISBN: ISBN })
        if (dupliIsbn) { return res.status(400).send({ Status: false, msg: "This ISBN already exists. Please enter a different ISBN" }) }
        filter["ISBN"] = ISBN
      }
      ...
      if (validation.valid(releasedAt)) {
        if (!validation.isValidDateFormat(releasedAt)) { return res.status(400).send({ Status: false, msg: "Please enter the date in correct YYYY-MM-DD format" }) }
...
const deleteBooks = async (req, res) => {
  ...
  try {
    ...
    if (!validation.isValidObjectId(id)) {
      return res.status(400).send({ Status: false, msg: "Please enter a valid id" })
    }

    let book = await bookModel.findById(id)

    if (!book) {
      return res.status(404).send({ Status: false, msg: "No book with this id found" })
    }
    if (book.isDeleted === true) {
      return res.status(400).send({ Status: false, msg: "The requeste book has alredy been deleteed" })
    }
    if (book.userId == req.decodedToken.userId) {

      let deletedBook = await bookModel.findOneAndUpdate({ _id: id }, { isDeleted: true, deletedAt: Date.now() }, { new: true })
