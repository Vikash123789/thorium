...
const createBooks = async (req, res) => {
  try {
    ...
    if (!validation.isValidObjectId(userId)) { return res.status(400).send({ status: false, msg: "Not  Valid Object Id", }); }
    const user = await userModel.findById(userId)
    if (!user) { return res.status(404).send({ Status: false, msg: "User with this id not found" }) }
    ...
    if (!validation.valid(releasedAt)) { return res.status(400).send({ Status: false, msg: "Please provide the realesdAt field and enter a date" }) }
